...
export class ChatInterfaceComponent implements OnInit, OnChanges, AfterViewChecked, OnDestroy {
  ...
  private buildSummaryPrompt(agentMessages: Message[], agentDisplayName: string): string {
    ...
    const prompt = `You are an AI assistant helping to summarize the key contributions of a specific agent in a multi-agent conversation about real estate personalization and home search.
...
Keep the summary concise but comprehensive, focusing on actionable insights and specific recommendations. Use bullet points for clarity and readability.`;
  ...
  private adcpToolResultDetectors: Array<{
  ...
  }> = [
    {
      ...
      messageText: (data) => `Found ${data.products?.length || 0} property listings`,
      detect: (parsed) => {
  ...
  private initializeChat(): void {
    ...
    this.messages = [{
      id: '1',
      text: `This playground shows various applications of Amazon Bedrock AgentCore Agents for Real Estate. Select one of the example scenarios or speak or type a question to get started.`,
      sender: 'agent',
      timestamp: new Date(),
      data: {
